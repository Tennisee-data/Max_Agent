...
class PDFGenerator:
    ...
    def setup_pdf(self):
        class PDF(FPDF):
            def header(self):
                ...
                self.cell(0, 10, 'Summarised PDF by Max_Agent', border=False, ln=True, align='C')
                self.ln(10)
        ...
        try:
            ...
            pdf.add_font('DejaVuSans', '', os.path.join(self.font_dir, 'DejaVuSans.ttf'), uni=True)
            pdf.add_font('DejaVuSans', 'B', os.path.join(self.font_dir, 'DejaVuSans-Bold.ttf'), uni=True)
            pdf.add_font('DejaVuSans', 'I', os.path.join(self.font_dir, 'DejaVuSans-Oblique.ttf'), uni=True)
            pdf.add_font('DejaVuSans', 'BI', os.path.join(self.font_dir, 'DejaVuSans-BoldOblique.ttf'), uni=True)
    ...
    def replace_pua_characters(self, text, replacement='#'):
        """
        Replaces all Private Use Area (PUA) characters in the text with a specified replacement character.
        
        Parameters:
        - text (str): The input text containing potential PUA characters.
        - replacement (str): The character to replace PUA characters with. Default is '#'.
