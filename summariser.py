...
class Summariser:
    ...
    def summarise_chunk(self, chunk, min_ratio, max_ratio, retries=3, delay=3):
        ...
        logging.info(f"Summarising chunk with {chunk_length} tokens (min: {dynamic_min_length}, max: {dynamic_max_length})")
    ...
    def summarise_chunks_parallel(self, chunks, min_ratio=0.20, max_ratio=0.35, max_workers=None):
    ...
    def summarise(self, chunks, first_min_ratio, first_max_ratio, second_min_ratio, second_max_ratio):
        # First summarisation tier
        logging.info("Starting first summarisation tier...")
        partial_summaries = self.summarise_chunks_parallel(
            chunks,
            min_ratio=first_min_ratio,
            max_ratio=first_max_ratio
        )
        ...
        if combined_length > self.tokenizer.model_max_length:
            logging.warning(f"Combined summary exceeds model's max length ({self.tokenizer.model_max_length} tokens). Performing second summarization tier...")
            dynamic_min_length = max(int(combined_length * second_min_ratio), 1)
            dynamic_max_length = max(int(combined_length * second_max_ratio), dynamic_min_length + 1)
            ...
            logging.info("Combined summary within token limits. Returning combined summary.")
