...
class PDFSummariserApp:
    def __init__(self, config_file='config.yaml'):
        ...
        self.pdf_processor = PDFProcessor(font_dir=self.config.font_directory, cleaned_text_dir=self.config.cleaned_text_directory)
        self.summariser = Summariser(model_id=self.config.summarization_model_id, device=0 if torch.cuda.is_available() else -1)
    ...
    def process_pdf(self, pdf_filename):
        ...
        output_pdf_path = os.path.join(self.config.output_directory, output_pdf_filename)
        cleaned_txt_filename = f"{name}_cleaned.txt"
        cleaned_txt_path = os.path.join(self.config.cleaned_text_directory, cleaned_txt_filename)
        ...
        logging.info(f"'{pdf_filename}' has been summarised by Max_Agent and saved as '{output_pdf_filename}'.")
    ...
    def run(self):
        ...
        all_pdfs = [f for f in os.listdir(self.config.input_directory) if f.lower().endswith('.pdf')]
